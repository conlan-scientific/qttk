...
def fiviz(x, y1, y2):
    ...
    y1_transposed = y1.T
    axs[0].boxplot(y1_transposed, whis='range')
...
def save_data(filename, df):
    ...
    filename = path+'\data\validation_data\{}.csv'.format(filename)
    df.to_csv(filename)
...
def rsi(rets, window):
    ...
    # replace 0s with 1s
    down_avg.replace(to_replace=0, value=1)
...
if __name__ == '__main__':
    ...
    # window needs to be optimized for trading frequency/price autocorrelation lag period
    window = 14
